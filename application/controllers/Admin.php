...
class Admin extends CI_Controller {
    ...
    {
        if ($this->uri->segment(3) === 'addclient') {

            $data = [
                'content' => 'admin/client/addclient',
                'navlink' => 'client',
            ];
            ...
            $tampil = $this->crudModel->getDataWhere('client', 'id_client', $id)->row();

            $data = [
                'detail' => [
                    'id_client' => $tampil->id_client,
                ...
                'content' => 'admin/client/updateclient',
                'navlink' => 'client',
            ]; 
    
        } else {
            $data = [
                'client' => $this->crudModel->getData('client')->result(),
                'content' => 'admin/client/client',
                'navlink' => 'client',
            ];
    ...
    {
            $add = [
                'id_client' => $this->crudModel->generateCode(1, 'id_client', 'client'),
            ...
            $this->crudModel->addData('client', $add);
    ...
    public function updateClient()
    {
            ...
            $this->crudModel->updateData('client', 'id_client', $id, $update);
    ...
    {
            $id = $this->uri->segment(3);
            $this->crudModel->deleteData('client', 'id_client', $id);
            redirect(site_url('admin/client'));
    }
    ...
    public function project()
    {
        if ($this->uri->segment(4) === 'view') {
            ...
            $data = [
                ...
                'content' => 'admin/project/projectview',
                'navlink' => 'project',
            ...
            $data = [
                ...
                'content' => 'admin/project/service',
                'navlink' => 'project',
            ];

        } else if ($this->uri->segment(3) === 'addservice') {

            $data = [
                'content' => 'admin/project/addservice',
                'navlink' => 'project',
            ];
            ...
            $data = [
                ...
                'client' => $this->db->query('SELECT * FROM client')->result(),
                'content' => 'admin/project/updateproject',
                'navlink' => 'project',
            ]; 

        }else if ($this->uri->segment(4) === 'updateservice') {

            $id = $this->uri->segment(3);

            $tampil = $this->crudModel->getDataWhere('service', 'id_service', $id)->row();

            $data = [
                'detail' => [
                    'id_service' => $tampil->id_service,
                    'service_name' => $tampil->service_name,
                    'description' => $tampil->description,
                ],

                'content' => 'admin/project/updateservice',
                'navlink' => 'project',
            ]; 


        } else if ($this->uri->segment(3) === 'addproject') {
    ...
    public function addProject()
    {
            $add = [
                ...
                'project_priod' => trim($this->input->post('project_priod')),
                'attachment' => trim($this->input->post('attachment')),
                'p_status' => trim($this->input->post('p_status')),
            ];

            $this->crudModel->addData('project', $add);

            redirect(site_url('admin/project/'.$add['id_project'].'/view'));
    }
    ...
    public function updateProject()
    {
        ...
        $this->crudModel->updateData('project', 'id_project', $id, $update);
    ...
    {
        $id = $this->uri->segment(3);
        $this->crudModel->deleteData('project', 'id_project', $id);
    ...
    {
        $add = [
            'id_service' => $this->crudModel->generateCode(1, 'id_service', 'service'),
            'service_name' => trim($this->input->post('service_name')),
            'description' => trim($this->input->post('description')),
        ];

        $this->crudModel->addData('service', $add);

        redirect(site_url('admin/project/service'));
    }
    ...
    public function updateService()
    {
        ...
        $this->crudModel->updateData('service', 'id_service', $id, $update);

        redirect(site_url('admin/project/service'));
    }

    public function proserv()
    {
        $id_project = $this->input->post('id_project');

        $add = [
            'id_proserv' => $this->crudModel->generateCode(1, 'id_proserv', 'proserv'),
            'id_project' => trim($this->input->post('id_project')),
            'id_service' => trim($this->input->post('id_service')),
        ];

        $project = $this->crudModel->getDataWhere('project', 'id_project', $add['id_project'])->row();

        $update = [
            'log' =>  $project->log + 1,
        ];
        
        $this->crudModel->addData('proserv', $add);

        $this->crudModel->updateData('project', 'id_project', $add['id_project'], $update);

        $this->session->set_flashdata('flash', 'ditambah');

        redirect(site_url('admin/project/'.$id_project.'/view'));
    }

    public function deleteProserv()
    {
        $id_project = $this->uri->segment(3);
        $id_service = $this->uri->segment(4);

        $this->crudModel->deleteData('proserv', 'id_service', $id_service);
    ...
    public function analyst()
    {
        if ($this->uri->segment(4) === 'view') {
            ...
            $data = [
                ...
                'content' => 'admin/analyst/analystview',
                'navlink' => 'analyst',
            ...
            $data = [
                ...
                'project' => $this->db->query('SELECT * FROM project')->result(),
                'content' => 'admin/analyst/addanalyst',
                'navlink' => 'analyst',
            ];
    ...
    public function addAnalyst()
    {
        ...
        redirect(site_url('admin/analyst/'.$id_project.'/view'));
    }
    ...
    public function staff()
    {
        if ($this->uri->segment(3) === 'addstaff') {
            ...
            $data = [
                'staff' => $this->db->query('SELECT * FROM staff JOIN team WHERE staff.id_team = team.id_team')->result(),
    ...
    public function updateStaff()
    {
            ...
            $this->crudModel->updateData('staff', 'id_staff', $id, $update);
    ...
    {
        if ($this->uri->segment(3) === 'addteam') {

            $data = [
                'content' => 'admin/team/addteam',
                'navlink' => 'team',
            ];
            ...
            $tampil = $this->crudModel->getDataWhere('team', 'id_team', $id)->row();

            $data = [
                'detail' => [
                    'id_team' => $tampil->id_team,
                    'division' => $tampil->division,
                ],

                'content' => 'admin/team/updateteam',
                'navlink' => 'team',
            ]; 
    ...
    {
            $add = [
                'id_team' => $this->crudModel->generateCode(1, 'id_team', 'team'),
                'division' => trim($this->input->post('division')),

            ];

            $this->crudModel->addData('team', $add);
    ...
    public function updateTeam()
    {
            ...
            $this->crudModel->updateData('team', 'id_team', $id, $update);
    ...
    public function schedule()
    {
        if ($this->uri->segment(4) === 'view') {
            ...
            $data = [
                ...
                'content' => 'admin/schedule/scheduleview',
                'navlink' => 'schedule',
                'analyst' => $this->db->query('SELECT * FROM analyst JOIN proserv JOIN project JOIN service JOIN team WHERE analyst.id_proserv = proserv.id_proserv AND proserv.id_project = project.id_project AND project.id_project = '.$id.' AND proserv.id_service = service.id_service AND analyst.id_team = team.id_team ORDER BY analyst.an_enddate ASC ')->result(),
            ...
            $data = [
                ...
                'proserv' => $this->db->query('SELECT * FROM project JOIN proserv JOIN service WHERE project.id_project = '.$id.' AND project.id_project = proserv.id_project AND proserv.id_service = service.id_service')->result(),
                'project' => $this->db->query('SELECT * FROM project')->result(),
                'content' => 'admin/schedule/addschedule',
                'navlink' => 'schedule',
            ];
            ...
            $data = [
                'project' => $this->db->query('SELECT * FROM project JOIN client WHERE project.id_client = client.id_client')->result(),
                'content' => 'admin/schedule/schedule',
                'navlink' => 'schedule',
                ];
        }
    ...
    public function addSchedule()
    {
        ...
        redirect(site_url('admin/schedule/'.$id_project.'/view'));
    }
    ...
    public function addTask()
    {
        ...
        redirect(site_url('admin/task/task'));
    }
    ...
    public function review()
    {
        if ($this->uri->segment(4) === 'view') {
            ...
            $data = [
                ...
                'content' => 'admin/review/reviewview',
                'navlink' => 'review',
                'analyst' => $this->db->query('SELECT * FROM analyst JOIN proserv JOIN project JOIN service JOIN team WHERE analyst.id_proserv = proserv.id_proserv AND proserv.id_project = project.id_project AND project.id_project = '.$id.' AND proserv.id_service = service.id_service AND analyst.id_team = team.id_team ORDER BY analyst.an_enddate ASC ')->result(),
                'schedule' => $this->db->query('SELECT * FROM schedule JOIN proserv JOIN project JOIN service JOIN team JOIN staff JOIN client JOIN task JOIN analyst WHERE schedule.id_task = task.id_task AND task.id_analyst = analyst.id_analyst AND task.id_staff = staff.id_staff AND analyst.id_team = team.id_team
                                                AND analyst.id_proserv = proserv.id_proserv AND proserv.id_project = project.id_project AND proserv.id_service = service.id_service AND project.id_client = client.id_client AND project.id_project = '.$id.' ORDER BY task.t_status DESC, schedule.end_time ASC ')->result(),
    ...
    public function finishTask()
    {
        ...
        redirect(site_url('admin/review/review'));
    ...
    public function progress()
    {
        if ($this->uri->segment(4) === 'view') {
            ...
            $data = [
                ...
                'content' => 'admin/progress/progressview',
                'navlink' => 'progress',
                'analyst' => $this->db->query('SELECT * FROM analyst JOIN proserv JOIN project JOIN service JOIN team WHERE analyst.id_proserv = proserv.id_proserv AND proserv.id_project = project.id_project AND project.id_project = '.$id.' AND proserv.id_service = service.id_service AND analyst.id_team = team.id_team ORDER BY analyst.an_enddate ASC ')->result(),
            ...
            $data = [
                ...
                'content' => 'admin/progress/progress',
                'navlink' => 'progress',
                ];
        }
    ...
    public function finishProject()
    {
        ...
        redirect(site_url('admin/report/report'));
    }
    ...
    public function report()
    {
        if ($this->uri->segment(4) === 'view') {
            ...
            $data = [
                ...
                'content' => 'admin/report/reportview',
                'navlink' => 'report',
                'analyst' => $this->db->query('SELECT * FROM analyst JOIN proserv JOIN project JOIN service JOIN team WHERE analyst.id_proserv = proserv.id_proserv AND proserv.id_project = project.id_project AND project.id_project = '.$id.' AND proserv.id_service = service.id_service AND analyst.id_team = team.id_team ORDER BY analyst.an_enddate ASC ')->result(),
